...
Доступные системы налогообложения физических лиц (PIT - Personal Income Tax) для JDG по состоянию на 2026 год:
...
Для уплаты налогов и сборов в Польше используется индивидуальный счёт (mikrorachunek). Идентификатор плательщика может быть NIP или PESEL. У ИП это всегда NIP!
...
- **Шаг 1.** Нужно разрешить e‑Correspondence, иначе в конце шага 8 вас ждет неприятный сюрприз - цикл без возможности продолжить отправку: ![e-Urząd Skarbowy: ошибка из-за отключённой e-Korespondencji][3]![Отправка зациклилась без возможности продолжить][4]Идем в `Ustawienia` > `Zgody i powiadomienia`, и привязываем email и/или телефон.
- **Шаг 2.** В боковом меню выбираем `Dokumenty` > `Złóż dokument`. И в списке документов находим `Zaświadczenie o niezaleganiu lub stanie zaległości w podatkach`. ![Dokumenty → Złóż dokument: выбор Zaświadczenie o niezaleganiu][5]
- **Шаг 3.** Выбираем тип справки - `o niezaleganiu w podatkach`, то есть справку о том что долгов нет. ![Выбор типа справки — o niezaleganiu w podatkach][6]
- **Шаг 4.** Выбираем для кого будет выдана справка - `za siebie`, то-есть для себя. ![Выбор, для кого справка — za siebie][7]
- **Шаг 5.** Выбираем какой идентификатор будет напечатан на справке - `PESEL` если вам справка нужна как для физического лица, или `NIP` соответственно как для юридического лица. От этого будет справке меняется одна строчка _Dane identyfikacyjne podatnika_ на выбранный вариант. Данные с адресом заполняются автоматически из того что вы подали в налоговую при регистрации JDG. ![Выбор идентификатора PESEL или NIP для печати на справке][8]
- **Шаг 6.** В свободной форме пишем, для чего берем справку, например - `w celu uzyskania karty pobytu`. Справа можно посмотреть примеры, если нажать на `Więcej`. Похоже, что на конечный результат это не влияет. ![Поле цели получения справки, примеры по ссылке Więcej][9]
- **Шаг 7.** Не обязательно, но можно оставить свой телефон и почту для связи. Похоже, этот пункт тоже не влияет на результат. ![Необязательные контактные данные — телефон и email][10]
- **Шаг 8.** Проверяем, что все данные вписаны верно и жмем кнопку `Akceptuj i wyślij` для отправки. В появившемся всплывающем окне еще раз подтверждаем нажатием кнопки `Wyślij wniosek`, что хотим отправить запрос. ![Сводка заявления и кнопка Akceptuj i wyślij][11] ![Подтверждение отправки кнопкой Wyślij wniosek][12]
- **Шаг 9.** После отправки запроса, будет написано, что рассмотрение произойдет в течение семи дней. Но похоже, что все полностью автоматизировано, и ответ приходит в течение пары минут, даже ночью и в выходные. Сообщение о готовности справки приходит на почту, а также в самом e-Urząd аккаунте. Чтобы скачать саму справку, справа в меню выбираем `Dokumenty` > `Dokumenty otrzymane`. Там будет список всех полученных документов, жмем на кнопку `Odbierz` и скачиваем pdf со справкой. Справка будет на двух страницах, ниже пример как она выглядит. ![Dokumenty otrzymane: кнопка Odbierz для скачивания справки][13] ![Пример готовой справки o niezaleganiu w podatkach][14]
...
1. Ваш облагаемый доход (сумма всех доходов от предпринимательской деятельности, которая фактически поступила на ваш счёт) превышает 240_000 злотых / год (лимит действует с 2026 года, ранее было 200_000) и фирма, с которой вы сотрудничаете, находится в Польше (польские заказчики). Если вы единожды превысили данный лимит, то становитесь плательщиком VAT, вне зависимости от дальнейших доходов.
...
!!! tip "Как проверить регистрацию?"
    Проверить, что ваш контрагент является VAT-EU плательщиком, можно в [базе VIES Еврокомиссии][17].
...
[5]: images/taxes/zaswiadczenie_00_create_document.jpg
[6]: images/taxes/zaswiadczenie_01_document_details.jpg
[7]: images/taxes/zaswiadczenie_02_who_concerns.jpg
[8]: images/taxes/zaswiadczenie_03_identification_data.jpg
[9]: images/taxes/zaswiadczenie_04_certificate_purpose.jpg
[10]: images/taxes/zaswiadczenie_05_contact_details.jpg
[11]: images/taxes/zaswiadczenie_06_summary.jpg
[12]: images/taxes/zaswiadczenie_07_submit_confirmation.jpg
[13]: images/taxes/zaswiadczenie_08_received_documents.jpg
[14]: images/taxes/zaswiadczenie_09_certificate_example.jpg
