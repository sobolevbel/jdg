...
1) Если вы отправили ZUS ZUA до обработки ZUS ZWUA, то можете видеть ошибку вроде такой:
...
2) При отправке декларации за предыдущий месяц (последний месяц ульги на старт), скорее всего при верификации декларации вы увидите статус Informacja, и в списке ошибок текст
Zbliża się koniec okresu ulgi (blok X pole 1) (само поле с кодом 0540 при этом будет подсвечено красным).
Так ZUS сообщает о том, что скоро закончится льгота и вы должны что-то с этим делать (это просто предупреждение).
Отправке декларации ZUS DRA эта "ошибка" верификации не мешает. 
