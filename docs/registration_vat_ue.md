...
![Rejestracja podatnika VAT UE: справка о титуле обязанности и поле даты начала внутрисоюзных транзакций][14]

Раньше здесь нужно было чек-боксами выбирать, чем именно вы собираетесь заниматься (товары или услуги), но CEIDG упростил форму: теперь это просто справка о том, что даёт регистрация VAT-UE, а указать нужно только одно — `Przewidywana data rozpoczęcia transakcji wewnątrzwspólnotowych`, то есть предполагаемую дату начала операций с контрагентами из EU.

Дата должна быть как минимум на 1 день позже даты подачи заявления.
Например, если заявление высылается 28 апреля, то самая ранняя дата — 29 апреля.
