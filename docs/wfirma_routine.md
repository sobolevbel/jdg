...
1. Выбираем контрагента, которому выставляем фактуру.
...
1. Выберите куда предоставляете услугу.
...
Если получаете оплату в валюте и продаете ее позже для расходов, связанных с предпринимательской деятельностью, например, для уплаты налогов, то также необходимо учесть [курсовую разницу][28] от środków własnych. Для рассчета воспользуемся [калькулятором][30] и методом LIFO (метод выбирайте себе сами).

1. Нажимаем Przychody.
2. Нажимаем Inne przychody.
3. Нажимаем Pozostałe przychody (DW).
4. Заполняем Nazwa przychodu. Примерный текст для положительной КР:
