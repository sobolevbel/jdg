...
> Важно: На этом этапе фактура еще не участвует в рассчете налоговой базы за выбранный месяц (пока она находится в статусе `SZKIC`). Для того чтобы фактура попала в зачет, необходимо ее напечатать или выслать клиенту. 
...
*(Подразумевается что вы знаете номер своего налогового счета. См [тут](taxes.md#Как узнать свой счет для оплаты налогов). Infakt получает номер счета автоматически при насттройке)*
...
Вторая опция платная, стоимость завсит от суммы оплаты (что-то в районе 3-5 зл сам налог). Однако она самая простая и удобная, необходимо просто нажать **Opłać z inFakt** и завершить платеж с помощью интересующих способов оплаты.
...
3. ввведите номер своего налогового счета
 ...
 *(Подразумевается что вы знаете номер своего миктосчета ZUS. См детали как настроить [тут](infakt_settings.md#Настройка данных ZUS))*
 ...
 Для оплаты напрямую через приложиение банка необходимо выполнить Przelew Krajowy, выбрать счет, с которого оплачивать (как писал выше, имеет смысл платить с фирмового):
...
Нажимаем `Wyślij`, подписываем документ (намример, profilem zaufanym ePUAP) и на этом все. Документ должен появиться в папке `Dokumenty wysłane`.
...
2. впивысаем `dodatnia różnica kursowa` или `ujemna różnica kursowa` в зависимости от курсовой разницы
...
Записывем изменения и документ сразу начинает участвовать в расчете налоговой базы.
